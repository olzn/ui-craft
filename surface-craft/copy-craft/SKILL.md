---
name: copy-craft
description: Write and review interface copy that guides, explains, reassures, or persuades. Use when drafting or improving error message bodies, empty states, helper text, onboarding text, tooltip prose, loading text, success or warning messages, confirmation body copy, and marketing-style CTA copy. Also triggers for UX writing, microcopy, tone, localisation notes, or "this copy feels unclear". Does NOT own product action labels, command names, menu items, feature names, product names, component names, token names, or UI terminology (use naming-craft). Does NOT decide placement or flow structure (use pattern-craft), component state coverage (use component-craft), or accessibility mechanics (use detail-craft).
---
...
**naming-craft names actions and concepts. copy-craft writes the surrounding language.** Keep this boundary clear. Do not override product action labels, command names, feature names, component names, token names, or glossary terms chosen by naming-craft.
...
- The action or concept names established by naming-craft.
...
If the vocabulary is unstable, use naming-craft first.
...
| Need | Lead |
|---|---|
| Product action labels, command names, menu items | `naming-craft` |
| Feature names, product names, UI terminology | `naming-craft` |
| Error message body text | `copy-craft` |
| Empty state title and body text | `copy-craft` with terms from `naming-craft` |
| Helper text, hints, onboarding, loading, success, warning, tooltip prose | `copy-craft` |
| Marketing-style CTA copy | `copy-craft` |
| Where feedback appears | `pattern-craft` |
| Whether a state exists | `component-craft` |

For full flows, sequence them:

```text
naming-craft -> pattern-craft -> copy-craft
```

naming-craft establishes vocabulary and action labels. pattern-craft decides placement and flow. copy-craft writes the explanatory and tonal layer.
...
The action label belongs to naming-craft. The body copy explains consequence and recovery.
...
Use copy-craft for persuasive CTAs outside routine product commands:
...
1. **Copy that renames the product model.** Do not introduce synonyms that fight naming-craft.
...
- Uses established terminology from naming-craft
