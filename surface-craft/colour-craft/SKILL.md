...
For token architecture and the three-layer model, see **token-craft**. For colour naming and palette terminology, see **naming-craft**. For theme switching implementation (disabling transitions), see **detail-craft**. For accessibility across all skills, see `accessibility.md`. For multi-skill task sequencing, see `composition.md`.
