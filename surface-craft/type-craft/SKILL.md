...
Type supports the Simplicity pillar from `design-philosophy.md`: clear hierarchy guides the eye and reduces cognitive load. For colour systems, use **colour-craft**. For token architecture, use **token-craft**. For multi-skill task sequencing, see `composition.md`.
...
Start from a base of 16px (1rem) for body text. Apply the ratio upward for headings and downward for captions. Store as design tokens following token-craft conventions:
