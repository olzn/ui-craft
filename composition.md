# Composition

How the eight skills across ui-craft and system-craft work together. Use this guide to determine which skills to invoke, in what order, for common tasks.

---

## The General Order

```
tokens → type + colour → components → patterns → detail + motion + interaction
```

Foundation scales first. Visual values next. Structure third. Assembly fourth. Polish last.

This is a dependency order, not a strict sequence. Later skills reference earlier ones, but you may loop back (e.g. adding a new token while building a component).

---

## Skill Sequence by Task

### New project setup

1. **token-craft** — spacing, radius, shadow, z-index, breakpoint, motion tokens
2. **type-craft** — type scale, font loading, font pairing
3. **colour-craft** — OKLCH palette, semantic colour mapping, light/dark themes
4. **component-craft** — base component conventions (prop names, CVA, forwarding)
5. **pattern-craft** — layout structure, navigation, feedback strategy

### Building a page

1. **pattern-craft** (lead) — layout pattern, navigation, data display, feedback
2. **component-craft** — components needed for the page
3. **colour-craft** — verify contrast in both themes
4. **type-craft** — verify scale, rhythm, and hierarchy
5. **detail-craft** — platform details, touch, scroll, performance
6. **motion-craft** — entrance animations, scroll reveals
7. **interaction-craft** — gesture decisions, frequency calibration

### Building a component

1. **component-craft** (lead) — API design, variants, states, composition
2. **token-craft** — verify tokens exist for all values used
3. **detail-craft** — platform quirks, accessibility micro-details
4. **motion-craft** — hover/press/focus transitions, entrance/exit
5. **colour-craft** — contrast check

### Design system audit

1. **token-craft** (lead) — hardcoded values, naming consistency, scale coverage
2. **component-craft** — API consistency, missing states, variant alignment
3. **colour-craft** — contrast compliance, colour blindness, theme coverage
4. **type-craft** — scale adherence, loading performance, feature usage
5. **pattern-craft** — feedback consistency, navigation structure, form validation

### Visual polish pass

1. **detail-craft** (lead) — concentric radii, optical alignment, shadows, selection
2. **motion-craft** — easing, duration, exit patterns, stagger
3. **interaction-craft** — frequency calibration, spatial consistency, staging
4. **colour-craft** — dark mode fine-tuning, brand colour adjustments

---

## Which Skill Leads

| Task type | Lead skill | Always also check |
|---|---|---|
| Token/scale setup | token-craft | colour-craft, type-craft |
| Colour palette or theme | colour-craft | token-craft |
| Type system | type-craft | token-craft |
| New component | component-craft | token-craft, detail-craft |
| Page layout | pattern-craft | component-craft, detail-craft |
| Form implementation | pattern-craft | component-craft, detail-craft |
| Navigation structure | pattern-craft | component-craft, interaction-craft |
| Data display (tables, lists) | pattern-craft | component-craft |
| Animation implementation | motion-craft | interaction-craft |
| Gesture design | interaction-craft | motion-craft, detail-craft |
| Platform bug or quirk | detail-craft | — |
| Accessibility review | See `accessibility.md` | All relevant skills |
| Design system audit | token-craft | All skills |
| Visual polish | detail-craft | motion-craft, colour-craft |
| Feedback (toast/inline/banner) | pattern-craft | detail-craft, motion-craft |

---

## Cross-Skill Boundaries

These are the most common boundary questions:

**"Should this animate?"** → interaction-craft decides. **"How should it animate?"** → motion-craft implements.

**"What shadow value?"** → token-craft owns the scale. **"Box-shadow vs border?"** → detail-craft recommends shadows.

**"What colour for dark mode?"** → colour-craft decides the mapping. **"How does theming work?"** → token-craft owns the mechanism.

**"Where should this error show?"** → pattern-craft decides placement. **"What does the error state look like?"** → component-craft defines the visual state.

**"Is this contrast sufficient?"** → colour-craft checks. **"Is this perceivable without colour?"** → colour-craft's colour blindness rules + detail-craft's accessibility micro-details.
