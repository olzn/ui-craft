...
The shared design voice for the UI Craft suite. Based on the work of [Benji Taylor](https://benji.org/family-values) and [Rauno Freiberg](https://rauno.me).
...
- Avoid unexplained instant show/hide. Use motion when it clarifies spatial change, but allow deliberate instant changes for high-frequency, keyboard, reduced-motion, or accessibility-sensitive interactions. (See **surface-interaction** for the decision and **surface-motion** for implementation.)
- Shared elements morph between states, never unmount and remount.
- Directional transitions match spatial logic. Forward moves right. Back moves left. (See **surface-interaction** for the reasoning.)
...
- Typography is a deliberate choice, not a default. Display and body fonts are paired with intention. (See **surface-typography**.)
- Colour is built from perceptual models, not HSL intuition. (See **surface-colour**.)
...
- **surface-motion** implements the Fluidity pillar.
- **surface-interaction** decides when and whether Fluidity applies.
- **surface-typography** builds the typographic foundation that Simplicity depends on.
- **surface-colour** builds the colour system that Delight expresses through.
- **surface-details** catches the platform-level details that undermine all three pillars.
- **system-naming** keeps the product and system vocabulary coherent across design, code, and copy.
- **system-tokens** structures the foundation scales that all visual decisions draw from.
- **system-components** structures individual components so all of the above remains consistent.
- **system-patterns** assembles components into coherent features and pages.
