---
name: token-craft
description: Set up and maintain design token architecture and foundation scales. Use when defining spacing, radius, shadow, z-index, breakpoint, or motion scales, establishing the primitive/semantic/component token model, naming tokens, configuring theming, or auditing hardcoded values. Also triggers for "design tokens", "CSS custom properties", "token naming", "semantic layer", "spacing scale", "theming", or "hardcoded values". Provides scale definitions and structural rules that distribute colour, type, and motion values across a system. Does NOT cover broad naming taxonomies, UI terminology, user-facing labels, feature names, or product names (use naming-craft). Does NOT cover component APIs (use component-craft), composite patterns (use pattern-craft), palettes (use colour-craft), or type scales (use type-craft).
---

# Token Craft

Design token architecture and foundation scales. Tokens are the distribution layer that ensures every component draws from the same well. colour-craft, type-craft, and motion-craft define the values. This skill defines how those values are structured, named, and distributed.
...
**Apply this when setting up a project's design foundations or auditing token consistency.** For broad naming and terminology decisions, use naming-craft. For multi-skill task sequencing, see `composition.md`.
...
For cross-surface naming decisions, controlled vocabulary, Figma variable naming, and token taxonomy audits, use naming-craft. This section defines the token architecture rule.
...
Remember the concentric radius rule from detail-craft: nested elements use `outer radius - padding` for their inner radius.
...
Duration and easing as tokens. The values come from motion-craft; these tokens distribute them.
...
For colour-specific theming decisions (dark mode surface values, brand colour adjustments), see colour-craft.
