...
**Apply this whenever creating or reviewing a component.** For broad component, icon, and user-facing label terminology, use naming-craft. For accessibility across all skills, see `accessibility.md`. For multi-skill task sequencing, see `composition.md`.
...
| **Hover** | Visual change on pointer (scoped to `@media (hover: hover)`, per detail-craft) |
| **Focus** | Visible focus ring on keyboard navigation |
| **Active/Pressed** | Momentary feedback during click/tap (scale 0.97, per motion-craft) |
| **Disabled** | Visually muted, non-interactive, with inline explanation (not tooltip, per detail-craft) |
...
Icon sizes come from the token-craft icon scale (`--icon-xs` through `--icon-xl`). Default size for inline icons is `--icon-md` (20px, 1.25rem).
...
- **Keyboard:** Enter, Space, Escape, Arrow keys as appropriate. Tab order follows visual order. Sequential lists use roving tabindex (detail-craft).
- **ARIA:** Semantic HTML first. Add ARIA only when native semantics are insufficient.
- **Focus:** Visible focus ring on keyboard navigation. Modals trap focus; return it on close. Focus rings follow border-radius (detail-craft).
- **Colour:** Never use colour as the sole indicator of state (colour-craft).
- **States:** Disabled elements have inline explanation, not tooltip (detail-craft).
