---
name: detail-craft
description: Catch the platform-specific implementation details and visual polish that make web interfaces feel professional. Use this skill when building forms, inputs, toggles, buttons, dropdown menus, scroll-linked behaviour, or touch interfaces. Also triggers when debugging platform-specific issues ("why does iOS zoom on this input", "hover is sticky on mobile", "there's a dead zone between these items"), reviewing interactive UI for production readiness, or any question about input behaviour, touch quirks, focus management, performance pitfalls, accessibility micro-details, or visual refinements like border radius nesting, optical alignment, shadow depth, and image outlines. Based on the Web Interface Guidelines by Rauno Freiberg and the interface details work of Jakub Królikowski. Does NOT cover animation values or easing (use motion-craft), gesture design decisions (use interaction-craft), typography systems (use type-craft), colour palettes or contrast (use colour-craft), or component APIs (use component-craft) and tokens (use token-craft).
---
...
**`aria-label` on icon-only elements.** Every icon button, icon link, and icon toggle needs an explicit label.
...
- Images have a subtle semi-transparent outline for containment
