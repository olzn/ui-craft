...
Structured summary of accessibility requirements across the ui-craft and system-craft suites. This is a cross-reference guide, not a duplication of content. Each section points to the skill that owns the rules.
...
- `aria-label` on every icon-only button, link, and toggle (detail-craft)
